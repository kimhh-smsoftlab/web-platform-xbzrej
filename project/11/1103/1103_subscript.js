class TableView {
     ...
     * @param {object[]} arData
     */
    createTableHead(arData) {
        let title, tr, td;
        tr = document.createElement("div");
        tr.className = "tr th"; //클래스를 어떻게 하면 사용자가 설정할 수 있을까? table_header

        for (title in arData[0]) {
            td = document.createElement("div");
            td.textContent = title;
            td.className = `td`; //item_${title} table_title

            tr.appendChild(td);
            /* 화살표 만드는 기능 */
            let i = document.createElement("i");
            i.className = `fas fa-chevron-down`; // icon class
            td.appendChild(i);
     ...
     * @param {object[]} arData
     */
    createTableBody(arData) {
        let tbody = document.createElement("div"),
            tr,
            td,
            object;
        tbody.className = "tbody";
        // while(this.element.lastElementChild){ //자식 노드 제거
        //     debugger;
        //     this.element.removeChild(this.element.firstChild)
        // }
        for (object of arData) {
            tr = document.createElement("div");
            tr.className = `tr`; // 클래스는 나중에 따로 설정하도록 body_item
            for (let key in object) {
                td = document.createElement("div");
                td.textContent = object[key];
                td.className = `td`; //body_${key} item_${key}
            ...
            success = arItem.find(function (checkbox) {
                ...
                value =
                    typeof value === "string"
                        ? value.toLowerCase()
                        : value + "";
                return checklist.searchType === 0
                    ? value.includes(input)
                    : value == input;
            });
            return success !== undefined; //success에 하나의 값이라도 있다면 return
        });
        this.setDisplay(arr); //최종 검색된 obj배열
    }
    setDisplay(arData) {
        let arElement = this.element.lastElementChild.children,
            findObj,
            childnode;
        Array.from(arElement).forEach(function (Element) {
            findObj = arData.find(function (object) {
                childnode = Element.firstElementChild;
                return childnode.textContent == object[childnode.dataset.key];
            });
            findObj !== undefined
                ? w3.showElement(Element, "flex")
...
(filter_data = filterData(jsondata, "pq_", true)),
    (table_El = w3.getElement("#table_div")),
    (table = new TableView(table_El, filter_data));
let btn = w3.getElement("#search_btn");
btn.addEventListener("click", clickSearchBtn);

function filterData(jsondata, text, doParse) {
    let srttext = JSON.stringify(
        jsondata,
        function (key, value) {
...
function clickSearchBtn() {
    ...
    if (txt.value == "") {
        ...
        table.searchData(txt.value, getCheck()); //검색어를 search 한다
        txt.value = "";
    }
}
