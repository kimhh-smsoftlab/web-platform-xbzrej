                    ...
                    var HyperList = (function () {
                        ...
                        function HyperList(element, userProvidedConfig) {
                            ...
                            var render = function render() {
                                ...
                                _this._renderAnimationFrame = window.requestAnimationFrame(
                                    render
                                );
                                ...
                                var diff = lastRepaint
                                    ? scrollTop - lastRepaint
                                    : 0;
                                if (
                                    !lastRepaint ||
                                    diff < 0 ||
                                    diff > _this._averageHeight
                                ) {
                        ...
                        _createClass(HyperList, [
                            ...
                            },
                            {
                                ...
                                value: function refresh(
                                    ...
                                    if (!element || element.nodeType !== 1) {
                                        throw new Error(
                                            "HyperList requires a valid DOM Node container"
                                        );
                                    }
                                    ...
                                    if (
                                        typeof config.useFragment !== "boolean"
                                    ) {
                                    ...
                                    if (!config.generate) {
                                        throw new Error(
                                            "Missing required `generate` function"
                                        );
                                    }

                                    if (!isNumber(config.total)) {
                                        throw new Error(
                                            "Invalid required `total` value, expected number"
                                        );
                                    }

                                    if (
                                        !Array.isArray(config.itemHeight) &&
                                        !isNumber(config.itemHeight)
                                    ) {
                                        throw new Error(
                                            "\n        Invalid required `itemHeight` value, expected number or array\n      ".trim()
                                        );
                                    } else if (isNumber(config.itemHeight)) {
                                        this._itemHeights = Array(
                                            config.total
                                        ).fill(config.itemHeight);
                                    } else {
                                    ...
                                    var value =
                                        config[isHoriz ? "width" : "height"];

                                    if (value) {
                                        var isValueNumber = isNumber(value);
                                        var isValuePercent = isValueNumber
                                            ? false
                                            : value.slice(-1) === "%";
                                        // Compute the containerHeight as number
                                        var numberValue = isValueNumber
                                            ? value
                                            : parseInt(
                                                  value.replace(/px|%/, ""),
                                                  10
                                        ...
                                        if (isValuePercent) {
                                            this._containerSize =
                                                (innerSize * numberValue) / 100;
                                        } else {
                                            this._containerSize = isNumber(
                                                value
                                            )
                                                ? value
                                                : numberValue;
                                    ...
                                    var scrollerHeight =
                                        config.itemHeight * config.total;
                                    var maxElementHeight = this
                                        ._maxElementHeight;
                            ...
                            },
                            {
                                ...
                                value: function _getRow(i) {
                                    ...
                                    if (!item || item.nodeType !== 1) {
                                        throw new Error(
                                            "Generator did not return a DOM Node for index: " +
                                                i
                                        );
                                    }
                            ...
                            },
                            {
                                ...
                                value: function _getScrollPosition() {
                                    ...
                                    if (
                                        typeof config.overrideScrollPosition ===
                                        "function"
                                    ) {
                            ...
                            },
                            {
                                ...
                                value: function _renderChunk(force) {
                                    ...
                                    var from = config.reverse
                                        ? this._getReverseFrom(scrollTop)
                                        : this._getFrom(scrollTop) - 1;

                                    if (
                                        from < 0 ||
                                        from - this._screenItemsLen < 0
                                    ) {
                                        from = 0;
                                    }
                                    ...
                                    // to the parent node
                                    var fragment = config.useFragment
                            ...
                            },
                            {
                                ...
                                value: function _computeScrollHeight() {
                                    ...
                                    var total = config.total;
                                    var scrollHeight =
                                        this._itemHeights.reduce(function (
                                            a,
                                            b
                                        ) {
                                            return a + b;
                                    ...
                                    var sortedItemHeights = this._itemHeights
                                        .slice(0)
                                        .sort(function (a, b) {
                                            return a - b;
                                        });
                                    ...
                                    var clientProp = isHoriz
                                        ? "clientWidth"
                                        : "clientHeight";
                                    var element = config.scrollContainer
                                        ? config.scrollContainer
                                        : this._element;
                                    var containerHeight = element[clientProp]
                                        ? element[clientProp]
                                        : this._containerSize;
                                    this._screenItemsLen = Math.ceil(
                                        containerHeight / averageHeight
                                    );
                            ...
                            },
                            {
                                ...
                                value: function _computeScrollPadding() {
                                    ...
                                    var padding = function padding(location) {
                                        var cssValue = styles.getPropertyValue(
                                            "padding-" + location
                                        );
